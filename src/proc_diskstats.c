    ...
    for(l = 0; l < lines ;l++) {
        ...
        if(d->do_io == CONFIG_ONDEMAND_YES || (d->do_io == CONFIG_ONDEMAND_ONDEMAND && (readsectors || writesectors))) {
            ...
            if(!st) {
                ...
                rrddim_add(st, "reads", NULL, d->sector_size, 1024, RRDDIM_INCREMENTAL, NULL);
                rrddim_add(st, "writes", NULL, d->sector_size * -1, 1024, RRDDIM_INCREMENTAL, NULL);
            }
        ...
        if(d->do_ops == CONFIG_ONDEMAND_YES || (d->do_ops == CONFIG_ONDEMAND_ONDEMAND && (reads || writes))) {
            ...
            if(!st) {
                ...
                rrddim_add(st, "reads", NULL, 1, 1, RRDDIM_INCREMENTAL, NULL);
                rrddim_add(st, "writes", NULL, -1, 1, RRDDIM_INCREMENTAL, NULL);
            }
            else rrdset_next_usec(st, dt);
        ...
        if(d->do_qops == CONFIG_ONDEMAND_YES || (d->do_qops == CONFIG_ONDEMAND_ONDEMAND && queued_ios)) {
            ...
            if(!st) {
                ...
                rrddim_add(st, "operations", NULL, 1, 1, RRDDIM_ABSOLUTE, NULL);
            }
            else rrdset_next_usec(st, dt);
        ...
        if(d->do_backlog == CONFIG_ONDEMAND_YES || (d->do_backlog == CONFIG_ONDEMAND_ONDEMAND && backlog_ms)) {
            ...
            if(!st) {
                ...
                rrddim_add(st, "backlog", NULL, 1, 10, RRDDIM_INCREMENTAL, NULL);
            }
            else rrdset_next_usec(st, dt);
        ...
        if(d->do_util == CONFIG_ONDEMAND_YES || (d->do_util == CONFIG_ONDEMAND_ONDEMAND && busy_ms)) {
            ...
            if(!st) {
                ...
                rrddim_add(st, "utilization", NULL, 1, 10, RRDDIM_INCREMENTAL, NULL);
            }
            else rrdset_next_usec(st, dt);
        ...
        if(dt) {
            if( (d->do_iotime == CONFIG_ONDEMAND_YES || (d->do_iotime == CONFIG_ONDEMAND_ONDEMAND && (readms || writems))) &&
                ...
                if(!st) {
                    ...
                    rrddim_add(st, "reads", NULL, 1, 1, RRDDIM_ABSOLUTE, NULL);
                    rrddim_add(st, "writes", NULL, -1, 1, RRDDIM_ABSOLUTE, NULL);
                }
                else rrdset_next_usec(st, dt);
            ...
            if( (d->do_io  == CONFIG_ONDEMAND_YES || (d->do_io  == CONFIG_ONDEMAND_ONDEMAND && (readsectors || writesectors))) &&
                ...
                if(!st) {
                    ...
                    rrddim_add(st, "reads", NULL, d->sector_size, 1024, RRDDIM_ABSOLUTE, NULL);
                    rrddim_add(st, "writes", NULL, d->sector_size * -1, 1024, RRDDIM_ABSOLUTE, NULL);
                }
                else rrdset_next_usec(st, dt);
            ...
            if( (d->do_util == CONFIG_ONDEMAND_YES || (d->do_util == CONFIG_ONDEMAND_ONDEMAND && busy_ms)) &&
                ...
                if(!st) {
                    ...
                    rrddim_add(st, "svctm", NULL, 1, 1, RRDDIM_ABSOLUTE, NULL);
                }
                else rrdset_next_usec(st, dt);
                    ...
                    if(major(buff_stat.st_dev) == major && minor(buff_stat.st_dev) == minor) {
                        ...
                        if(d->do_space == CONFIG_ONDEMAND_YES || (d->do_space == CONFIG_ONDEMAND_ONDEMAND && (space_avail || space_avail_root || space_used))) {
                            ...
                            if(!st) {
                                ...
                                rrddim_add(st, "avail", NULL, 1, 1024*1024*1024, RRDDIM_ABSOLUTE, NULL);
                                rrddim_add(st, "used" , NULL, 1, 1024*1024*1024, RRDDIM_ABSOLUTE, NULL);
                                rrddim_add(st, "reserved_for_root", "reserved for root", 1, 1024*1024*1024, RRDDIM_ABSOLUTE, NULL);
                            }
                            else rrdset_next_usec(st, dt);
                        ...
                        if(d->do_inodes == CONFIG_ONDEMAND_YES || (d->do_inodes == CONFIG_ONDEMAND_ONDEMAND && (inodes_avail || inodes_avail_root || inodes_used))) {
                            ...
                            if(!st) {
                                ...
                                rrddim_add(st, "avail", NULL, 1, 1, RRDDIM_ABSOLUTE, NULL);
                                rrddim_add(st, "used" , NULL, 1, 1, RRDDIM_ABSOLUTE, NULL);
                                rrddim_add(st, "reserved_for_root", "reserved for root", 1, 1, RRDDIM_ABSOLUTE, NULL);
                            }
                            else rrdset_next_usec(st, dt);
