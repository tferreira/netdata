...
var example_chart = {
	...
	dimensions: {
		'dim1': {
			...
			color: '#0066FF'		// the html color code
		...
		'dim2': {
			...
			multiplier: 1,			// the multiplier
			divisor: 1,				// the divisor
			color: '#FF0066'		// the html color code
			hidden: false,			// is hidden (boolean)
		}
	...
	service: function(service) {
		...
		service._send_chart_to_netdata = function(chart) {
			...
			for(var dim in chart.dimensions) {
				...
				this.queue('DIMENSION "' + d.id + '" "' + d.name + '" "' + d.algorithm + '" ' + d.multiplier.toString() + ' ' + d.divisor.toString() + ' ' + ((d.color !== '')?d.color:'').toString() + ' ' + ((d.hidden === true)?'hidden':'').toString());
				d._created = true;
				d._updated = false;
			}
		...
		service.chart = function(id, chart) {
			...
			if(typeof(chart.dimensions) !== 'undefined') {
				for(var x in chart.dimensions) {
					...
					if(typeof(dim.color) !== 'undefined' && d.color !== dim.color) {
						if(netdata.options.DEBUG === true) netdata.debug('chart ' + id + ', dimension ' + x + ' updated its color');
						d.color = dim.color;
						d._updated = true;
					}
